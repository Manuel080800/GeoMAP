...
let mapBk = L.map('map').setView([23.466302332191862, -102.1152141635831], 5) //nos permite ver el mapa
...
function resetMap () {
  for (let index = 0; index < capMap.length; index++) {
    ...
    capMap[index]['data'].removeFrom(mapBk);
                    ...
                    '<div class="modal-dialog modal-dialog-centered" role="document">' +
                        '<div class="modal-content">' +
                            ...
                            '<div class="modal-footer">' +
                              '<button type="button" class="btn btn-primary"' + " onclick='" + 'reset_map(1)' + "'>Solo marcadores</button>" +
                              '<button type="button" class="btn btn-secondary"' + " onclick='" + 'reset_map(2)' + "'>Todo el mapa</button>" +
                            '</div>' +
                        '</div>' +
                    '</div>' +
                  '</div>';
...
try {
  ...
  if (user_share_ID) {
    ...
    try {
      $.post("http://localhost:63342//geomap//connection.php",
...
//tenemos que eliminar y agregar
 ...
 function drawItemSelectShare(option, selected, enableData, colorData, colorGUIData, finShare) { //para saber de que lista viene 
  ...
  try {
    $.post("http://localhost:63342//geomap//cerounoC.php", // Cual es la url de nuestro web service
...
function addComponents () { //este formatea primero el mapa
  ...
  for (let index = 0; index < capMap.length; index++) {
    ...
    structure = '<div class="input-group mb-3">' +
                  '<div style="display: flex; width: 100%">' +
                    '<div class="input-group-prepend">' +
                      '<div class="input-group-text" style="background-color: ' + capMap[index]['colorGUI'] + '; cursor: pointer;" onclick=' + '"seeCapSelect(' + "'" + capMap[index]['name'] + "', " + index + ')">' +
                    ...
                    '<div style="display: flex; justify-content: space-between; width: 100%; background-color: #FFFFFF; padding: 10px; border: 1px solid #ced4da;">' +
                      ...
                      "</span>" +
                      "<div "  + 'style="display: flex; width: 50%; justify-content: flex-end; align-items: center;"' + "><img src='https://upload.wikimedia.org/wikipedia/commons/thumb/3/33/OOjs_UI_icon_clear-destructive.svg/1200px-OOjs_UI_icon_clear-destructive.svg.png' alt='twbs' width='32' height='32' class='rounded-circle flex-shrink-0' onclick='" + 'deleteGUI(' + index + ')' + "' style='cursor: pointer;'></div>" +
                    "</div>" +
                  '</div>' +
                '</div>';
    ...
    if (capMap[index]['enable'] == true) {
      capMap[index]['data'].addTo(mapBk);//agregamos al mapa
    }
  }
...
function compartir_ubicacion () {
  fetch('http://localhost:63342//geomap//datos.txt').then(res => res.text()).then(content => {
...
function obtener_ubicacion () {
  var template = '<div class="modal fade" id="message-modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">' +
                    '<div class="modal-dialog modal-dialog-centered" role="document">' +
                        '<div class="modal-content">' +
                            '<div class="modal-header">' +
                            ...
                            '<div class="modal-body">' +
                              ...
                              "<input type='range'  style='width: 100%' class='form-range' min='100' max='1000' value='500' id='ranger_radio' oninput='updateRange(this.value)'>" +
                            '</div>' +
                            '<div class="modal-footer">' +
...
function ubicarMapa() {
  ...
  mapBk.locate({enableHighAccuracy:true});
  mapBk.once('locationfound', e => {
    ...
    mapBk.addLayer(marcador);

    console.log(circle.getBounds())

    // Agrega el zoon automático en el mapa
    mapBk.fitBounds(circle.getBounds());
...
function loadMap (Lat1, Lat2, Lon1, Lon2, showLoad) {
  try {
    // Reseteamos el mapa y datos. cuado hacemos otro cuadrito
      ...
      var dos = null;
      // Mandamos a llamar wl web service por post (que es lo mismo que ajax)
      $.post("http://localhost:63342/geomap/0.1.php", // Cual es la url de nuestro web service
  ...
  var template = '<div class="modal fade" id="exampleModalCenter" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">' +
                    '<div class="modal-dialog modal-dialog-centered" role="document">' +
                        '<div class="modal-content">' +
                            ...
                            '</div>' +
                            '<div class="modal-footer">' +
                              '<button type="button" class="btn btn-primary"' + "onclick='" + 'deleteCap(' + index + ')' + "'>Aceptar</button>'" +
                              '<button type="button" class="btn btn-secondary" data-dismiss="modal">Cancelar</button>' +
                            '</div>' +
                        '</div>' +
                    '</div>' +
...
mapBk.on('draw:created', function (e) {
  try {    
...
// Agregar escala
 new L.control.scale({imperial: false}).addTo(mapBk);
